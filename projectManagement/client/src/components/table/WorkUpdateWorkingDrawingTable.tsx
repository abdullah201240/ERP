...
interface WorkingDrawingTableProps {
...
const WorkUpdateWorkingDrawingTable: React.FC<WorkingDrawingTableProps> = ({ projectId  }) => {
    ...
    const fetchEmployeeProfile = useCallback(async () => {
      ...
      const fetchProjects = useCallback(async () => {
        ...
        try {
            let apiUrl = `${process.env.NEXT_PUBLIC_API_URL}projects/designPlan?projectId=${projectId}&stepType=Working&employeeId=${employeeDetails.id}`;
    
            if (accessData.some(access => access.permission_id === 16)) {
                apiUrl = `${process.env.NEXT_PUBLIC_API_URL}projects/designPlan?projectId=${projectId}&stepType=Working`;
            }
...
export default WorkUpdateWorkingDrawingTable;