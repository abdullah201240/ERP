...
export default function UpdateProjectPage() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const projectResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}siteVisit/project-site-visit-plan/${pageId}`, {
          headers: {
            Authorization: token,
          },
        });
  ...
  const handleSelectEmployee = async (employee: { id: string; name: string; }) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}siteVisit/assignedToProject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token,
        },
        body: JSON.stringify({ projectSiteVisitPlanId: pageId, eid: employee.id, eName: employee.name }),
      });
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}siteVisit/project-site-visit-plan/${pageId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token,
      ...
      toast.success('Project updated successfully!');
      router.push('/create-project-plan'); // Redirect to the projects page after successful update
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred';
      toast.error(errorMessage);
      setError(errorMessage);
  ...
  return (
    <div className="container mx-auto max-w-6xl mb-16">
      <h1 className="text-2xl text-black mb-6">Update  Project Plan</h1>
