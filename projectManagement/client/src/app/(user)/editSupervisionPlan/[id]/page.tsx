...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const projectResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}siteVisit/supervision-site-visit-plan/${pageId}`, {
          headers: {
            Authorization: token,
          },
        });
  ...
  const handleSelectEmployee = async (employee: { id: string; name: string; }) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}siteVisit/assignedToSupervision`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token,
        },
        body: JSON.stringify({ supervisionSiteVisitPlanId: pageId, eid: employee.id, eName: employee.name }),
      });
      if (!response.ok) {
  ...
  const handleRemoveEmployee = async (index: number, id: string) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}siteVisit/assignedToSupervision/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': token,
        },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}siteVisit/supervision-site-visit-plan/${pageId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token,
      ...
      if (!response.ok) {
        ...
        throw new Error('Failed to update Supervision');
      }
      toast.success('Supervision updated successfully!');
      router.push('/create-supervision'); // Redirect to the projects page after successful update
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred';
      toast.error(errorMessage);
      setError(errorMessage);
  ...
  return (
    <div className="container mx-auto max-w-6xl mb-16">
      <h1 className="text-2xl text-black mb-6">Update  Supervision Plan</h1>
      <div className='bg-[#433878] p-8 rounded-xl'>
        <form className="grid grid-cols-1 gap-6" onSubmit={handleSubmit}>
          <p className='text-white'>Fill-up all Information and create a update Supervision successfully!</p>
