...
export const verifyJWT = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            const sisterConcern = await SisterConcern.findOne({ where: { id: payload.id } });

            if (!sisterConcern) {
                return next(
                    new ApiError(
                        "Unauthorized request: Sister Concern not found",
                        401,
                        ErrorCodes.UNAUTHORIZED.code
                    )
                );
            ...
            (req as Request & { sisterConcern: SisterConcern }).sisterConcern = sisterConcern;
