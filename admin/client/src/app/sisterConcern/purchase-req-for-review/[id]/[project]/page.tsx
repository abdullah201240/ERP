...
export default function Page() {
    ...
    const fetchDrawings = useCallback(async () => {
        try {
            ...
            if (data.data) {
                setOperatingExpense(data.data.operatingExpense || "");
                setVat(data.data.vat || "");
                setTax(data.data.tax || "");
                setMargin(data.data.margin || "");
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_PROJECTMANAGEMENT}workingDrawing/update-vat-tax/${project}`, {
                ...
                body: JSON.stringify({ operatingExpense, vat, tax ,margin}),
            });
                        ...
                        <Table>
                            <TableHeader className='bg-[#2A515B] text-white'>
                                <TableRow className='text-center'>
                                    ...
                                    <TableHead className='text-white text-center'>TAX  Amount</TableHead>
                                    <TableHead className='text-white text-center'>After 
                                    TAX Amount</TableHead>

                                    <TableHead className='text-white text-center'>VAT</TableHead>
                                    <TableHead className='text-white text-center'>Total Amount</TableHead>
                            ...
                            <TableBody>
                                {materialList.length === 0 ? (
                                    ...
                                    materialList.map((material, index) => {
                                        ...
                                        const sourceTotal = Number(material.sourcePrice) * Number(material.itemQuantity);
                                        ...
                                        return (
                                            <TableRow key={material.id} className='text-center'>
                                                ...
                                                <TableCell className='border border-[#e5e7eb]'>{taxAmount.toFixed(2)}</TableCell>
                                                <TableCell className='border border-[#e5e7eb]'>{finalTotalwithTax.toFixed(2)}</TableCell>

                                                
                                                <TableCell className='border border-[#e5e7eb]'>{vatAmount.toFixed(2)}</TableCell>
                                                <TableCell className='border border-[#e5e7eb]'>{finalTotalwithAll.toFixed(2)}</TableCell>
